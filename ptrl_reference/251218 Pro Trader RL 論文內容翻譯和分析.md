...
這段文字是該篇論文的**摘要（Abstract）**，它概述了整個研究的核心目標、方法架構以及實驗結果。
...
三個實驗的結果顯示，無論市場狀況如何，該框架都能實現**高報酬**和**高夏普比率（Sharpe ratio）**，並具有穩定的表現以及**低最大回撤（Maximum Drawdown, MDD）**，其表現優於目前最先進（state-of-the-art）的研究成果。
...
另一方面，傳統的**資產配置**研究則側重於根據各種資產之間的相關性和預期報酬來平衡風險與回報 (Markowitz, 1952)。然而，這些傳統方法一直難以應對股票市場的複雜性、不規律性和快速變化 (Lo, 2004)。特別是，它們無法充分反映現代市場的特徵，例如全球化、技術進步和金融工具的多樣化 (Stulz, 2009)。
...
儘管有各種演算法和模型與市場數據互動以探索最佳交易策略 (Bao et al., 2017)，但現有的基於 RL 的股票交易研究在**全面整合專業交易員的交易哲學和風險管理策略方面仍受限**。

【Pro Trader RL 的解決方案】

本研究提出的「專業交易員 RL」(Pro Trader RL) 框架，旨在超越傳統 RL 方法在金融交易中的局限性。不同於以往的研究將決策過程簡化並合併為單一模型，Pro Trader RL 細緻地複製了專業交易員複雜且細微的決策模式。

這個框架不僅利用 RL 來模擬交易動作，更是專門圍繞著專業人士使用的**複雜交易哲學、投資組合管理策略和風險管理技術**而設計的。它透過構建 RL 系統的內部組件，獨立地模擬交易員決策過程中不同的面向來實現這一點。

**【Pro Trader RL 的具體特徵】**

* **輸入狀態 (State Inputs)：** Pro Trader RL 不將狀態輸入局限於特定的股票或金融指標。相反，它涵蓋了更廣泛的市場數據和交易員洞察，代表了專業交易員通常考量的整體觀點。  
* **獎勵機制 (Rewards)：** 獎勵不僅僅依賴於利潤或資產價值，而是**相對於各種可能的結果來定義**，這與交易員評估不同交易情境的方式密切一致。這種方法允許模型像專業交易員一樣從特定案例中學習，強調同時從成功和不理想的結果中吸取經驗。  
* **動作 (Actions)：** Pro Trader RL 中的動作不僅僅是買、賣或持有——它們涉及在每個決策點融入交易員交易哲學的**複雜判斷**，模仿經驗豐富的交易員的即時決策過程。  
* **風險管理 (Stop-loss)：** 此外，該框架整合了一個直接源自專業交易常見做法的**停損規則**，確保風險管理不是事後的補救想法，而是決策過程中的基礎組成部分。
...
1. **模組化架構：** Pro Trader RL 獨特地將專業交易員決策過程的每個部分，建構為獨立且相互連結的模組。這種新穎的方法允許從市場分析到交易執行的每個環節被**單獨優化**，從而提高整體交易效率。  
2. **量身打造的學習：** 該框架提供了一個強化學習環境與代理人（Agent），從專業交易員的角度分割決策過程，並對每個過程應用**量身定制（tailored）的學習方法**。這種客製化確保策略能最佳地適應不同交易決策的特定需求。  
...
4. **整合風險管理：** 透過將先進的風險管理協議直接整合到 RL 模組中，Pro Trader RL 不僅尋求利潤最大化，還強調透過有效管理潛在財務風險來實現**永續交易（sustainable trading）**。  
...
* **如果您時間有限：** 我建議我們可以**跳過 Section 2**，直接進入 **Section 3 (Pro Trader RL Framework)**，那裡是乾貨最多的地方，會詳細講模型怎麼設計。  
...
* **關鍵差異：** 差異在於專業交易員擁有**獨特的交易哲學**和**經驗**，這讓他們能靈活運用這些規則。此外，他們有**個人化的風險管理策略**來限制整個投資組合的損失。  
* **本研究的假設與限制：**  
  * Pro Trader RL 目前主要使用**技術指標**和**價格數據**（因為這些數據最直接、好量化）。  
...
1. **股票交易策略 (Stock Trading Strategies)：**  
   ...
   * **差異點 (關鍵)：** Ye 的方法是用「模仿學習」(Imitation Learning) 來學單一決策流程（買/賣/持有混在一起）。**本研究 (Pro Trader RL)** 則是將決策過程**拆解**成多個互聯的 RL 模組（買是買，賣是賣），更能精細複製專業交易員的步驟。雖然這使得模型更複雜、更難修改，但模擬得更真。  
...
1. **他承認目前的不足：** 在 2.1 節最後，他很誠實地說目前的模型**只看技術指標**（Technical Analysis），沒看基本面。這是一個很重要的前提。如果這套系統在基本面發生重大變化（如公司財報暴雷）時，可能會因為只看線圖而反應不及。這也是未來您可以改進的方向（加入財報數據）。  
...
數據是從 Yahoo Finance 每日收集的，任何缺乏至少兩年數據（無法生成技術指標）的股票都會被排除。經過這個嚴格的篩選過程，最終共有 **1,465 檔股票**用於我們的分析。
...
* 作者並不是讓 AI 從零開始看圖。他是先用一個經典的、很老的策略（唐奇安通道，也就是俗稱的「突破 20 日新高買進」）來做**第一層過濾**。  
* **為什麼這樣做？** 因為強化學習如果沒有方向，收斂會很慢。用一個經典策略作為「觸發器 (Trigger)」，AI 只需要專注於判斷：「這個突破是真的還是假的？」(Buy Knowledge RL) 以及「買進後什麼時候跑最好？」(Sell Knowledge RL)。  
* 這是一個很聰明的**降維打擊**，把「隨時都要判斷買不買」的問題，簡化成「當突破發生時，判斷 Yes or No」的問題。

**2\. 數據集的廣度**

* 包含了 S\&P 500, 400, 600，共 1465 檔股票。這代表模型**通用性**很強，不是只針對像 Apple 或 NVDA 這種大股票訓練的，也包含了波動大的小型股。這對於訓練 AI 的魯棒性 (Robustness) 很有幫助。

**3\. 關鍵的 "120 天" 窗口**

* 在 Sell Knowledge RL 中，作者提到鎖定買入後的 **120 天**。這暗示了這個模型的交易頻率屬於**波段交易 (Swing Trading)** 到 **中期趨勢交易**，而不是當沖 (Day Trading)。因為 120 天大約是半年的交易日，它在看的是一個中長期的波段。
...
表格 1、2、3 和 4 詳細列出了用於配置 RL 環境的變數清單。這些變數源自所有股票數據，將作為未來的輸入變數。這 69 個變數被分為四類：**基本變數**、**技術指標變數**、**股價指數變數**以及**股價指數與個股對比變數**。
...
這一節展示了作者對於**特徵工程**的深度。這不是隨便丟個 OHLCV (開高低收量) 給 AI 就算了，他精心設計了許多「相對」指標。
...
* 很多 AI 交易模型失敗的原因是「覆巢之下無完卵」。如果大盤崩盤，好股票也會跌。作者引入了 **RS (相對強度)** 系列變數，這是在模仿著名的 **CAN SLIM 投資法則**或 **威廉歐尼爾 (William O'Neil)** 的強勢股邏輯：**不只要漲，還要漲得比大盤兇**。  
...
儘管像 MinMax Scaler（最小最大縮放）、Standard Scaler（標準縮放）、MaxAbs Scaler 和 Robust Scaler 等通用方法很常用，但它們**並不適合股票數據**，因為它們只能在特定範圍內標準化數據。股票數據通常表現出極端的波動；例如，股價可能從 1 美元漲到 1000 美元，或跌至 0.1 美元。

在本研究中，我們應用了**專為股票數據量身定制的特定標準化公式**，以確保不同時間段的一致性。這些公式考慮了股票數據中常見的顯著變化，將數據值維持在定義的範圍內。

【公式類別 1：基於當日價格的相對標準化 (Eq 1-8)】

定義於公式 (1)–(8) 的方法透過利用其他變數的差異來標準化數據。

* $$DonchianUpper\_{new} \= \\frac{DonchianUpper}{High}$$  
   (1)  
* $$DonchianLower\_{new} \= \\frac{DonchianLower}{Low}$$  
   (2)  
* **注意：** 圖片中的公式 (3) 到 (8) 顯示的右側公式似乎完全相同（皆為 $\\frac{DonchianUpper}{High}$），這在學術論文中極可能是排版或複製貼上的錯誤（Typo）。  
  ...
  * (7) $HALow\_{new}$ ...  
  * (8) $HAHigh\_{new}$ ...  
  * *(正常邏輯推測：這些應該是各自的變數除以 High，例如 $Close\_{new} \= \\frac{Close}{High}$，或者是除以 DonchianUpper，否則這六個變數輸入給 AI 的數值會一模一樣，失去意義。)*

【公式類別 2：基於時間變化的比率 (Eq 9-10)】

定義於公式 (9)–(10) 的方法利用與「前一日數據」的差異來標準化數據。其中 $t$ 代表當前時間，$t-1$ 代表前一天。

* $$DJIATR\_{new} \= \\frac{DJIATR\_{t}}{DJIATR\_{t-1}}$$  
   (9)  
* $$ATR\_{new} \= \\frac{ATR\_{t}}{ATR\_{t-1}}$$  
   (10)  
...
* $$Index\_{new} \= \\frac{Index \- Index\_{\\min}}{Index\_{\\max} \- Index\_{\\min}}$$  
   (11)  
* $$Stock\_{new} \= \\frac{Stock \- Stock\_{\\min}}{Stock\_{\\max} \- Stock\_{\\min}}$$  
   (12)  
* $$AVGStock\_{new} \= \\frac{AVGStock \- Stock\_{\\min}}{Stock\_{\\max} \- Stock\_{\\min}}$$  
   (13)  
* $$RS\_{new} \= \\frac{RS \- RS\_{\\min}}{RS\_{\\max} \- RS\_{\\min}}$$  
   (14)  
* $$RSAVG\_{new} \= \\frac{RSAVG \- RS\_{\\min}}{RS\_{\\max} \- RS\_{\\min}}$$  
   (15)
...
* $$RSRate\_{new} \= RSRate \\times 0.01$$  
   (16)  
* $$MFI\_{new} \= MFI \\times 0.01$$  
   (17)  
* $$RSI\_{new} \= RSI \\times 0.01$$  
   (18)  
...
* 論文寫著：$Close\_{new} \= \\frac{DonchianUpper}{High}$  
* 論文寫著：$Low\_{new} \= \\frac{DonchianUpper}{High}$  
* 這在邏輯上是不通的。如果按照論文原本的公式寫程式，您的 $Close\_{new}$、$Low\_{new}$、$High\_{new}$ 數值將會**完全一模一樣**。  
* **正確的推測：** 作者的原意應該是「除以當日的 High」來做正規化。例如：  
  * $Close\_{new} \= \\frac{Close}{High}$  
  * $Low\_{new} \= \\frac{Low}{High}$  
...
* **公式 (19):**  
   $$SignalReturn \= \\frac{SellSignal\_{Open(t+1)} \- BuySignal\_{Open(t+1)}}{SellSignal\_{Open(t+1)}}$$  
  * *(註：這裡分母應該是 Buy Price 吧？通常報酬率公式是 (賣-買)/買。如果分母是賣價，計算出來的意義會變成「獲利佔賣出金額的比例」，這比較少見。我們可以標記這是一個潛在的疑點。)*

這個計算出的回報率被獎勵函數用來決定獎勵，而對應於買入訊號的**標準化數據**則作為**狀態 (State)** 提供給 RL Agent。

【定義成功與失敗：10% 門檻】

專業交易員基於過去失敗與成功的交易建立買入知識。區分失敗與成功的標準取決於每個交易員的哲學。大多數人認為回報率超過一定比例的交易是成功的。

為了將此應用於 RL，我們定義了**動作 (Action)** 和 **獎勵函數 (Reward Function)**：
...
* **數據平衡：** 為了避免學習偏差，回報率 ≥ 10% 的買入訊號與未達到此回報的訊號，會以**相同的比例**使用。這確保了模型不會因為市場上大部分股票都不會漲 10%，而傾向於永遠猜「不會漲」。
...
* **Top 10 策略：** Agent 輸出每檔股票回報 ≥ 10% 的機率。系統會將同一日期的股票匯總，優先選擇機率較高的股票，最終選出**前 10 名**機率最高的股票進行買入。

---

### **解析與討論：這裡隱藏了幾個關鍵的「Know-How」**

這一段揭露了這個模型的本質，它其實**不像傳統的 RL**（在每一步決定買或賣），而更像是一個**監督式學習分類器 (Supervised Classifier)** 被包裝在 RL 的框架裡。
...
在此階段，Agent 根據給定的**狀態 (State)** 和**獎勵 (Reward)**，在兩種行為中做出決定：預測（回報率）為 10% 或以上，以及預測小於 10%。共有兩個動作 (Actions)，一個對應於 10% 或以上的機率，另一個對應於小於 10% 的機率。

學習過程基於 **近端策略優化 (Proximal Policy Optimization, PPO)** 演算法，該演算法將從 RL 環境接收到的**標準化狀態**作為輸入，並輸出兩個機率值，每個值皆為 0 到 1 之間的小數。

本研究中使用的**策略網路 (Policy Network)** 結構基於**深度神經網路 (DNN)**，由三個層級組成，分別具有 **69、40 和 2 個單元**。**Actor Network (行動者網路)** 和 **Critic Network (評論家網路)** 的結構是相同的。
...
* **這是什麼？** PPO 是由 OpenAI 提出的一種強化學習演算法，是目前的**主流首選 (State-of-the-Art)**。  
...
$$SellReturn \= \\frac{Open\_{(t)}}{BuySignal\_{Open(t+1)}}$$  
...
* **訓練模式 (Train Mode)：** 使用買入訊號後 120 天的完整數據。為了聚焦於獲利，我們**排除**了那些最終沒有達到 10% 回報率的數據。目標是生成買入後 120 天內的賣出與持有機率。  
...
除了單純的賣出獲利，專業交易員還透過停損規則來管理風險。由於 Sell Knowledge RL 雖然具備在最佳收益率賣出的知識，但**缺乏獨立的風險管理功能**，因此這個缺口由停損規則來填補。因為停損需要一致的規則而非主觀判斷，我們實施了**基於規則 (Rule-based)** 的方法。

停損規則模組包含兩個子模組，與 Sell Knowledge RL **並行運作**，且其結果會**覆蓋 (Override)** Sell Knowledge RL 的決策：

1. **下跌停損 (Stop Loss on Dips)：** 如果任何時間點的回報率跌破 **\-10%**，將在隔天的開盤價執行停損，以管理風險。  
2. **盤整停損 (Stop Loss on Sideways)：** 如果在 120 天的交易期間內，偵測到**連續 20 天**回報率都在 10% 以下（且未觸發下跌停損），將在第 21 天的開盤價執行停損（時間停損）。
...
* 這代表 AI 要**超級確定**（信心度極高）才會賣。這導致的結果可能是：AI 會傾向於抱單 (Hold)，直到訊號非常強烈。這對於吃完整個波段很有幫助，但也可能導致坐雲霄飛車（獲利回吐）。
...
2. **實驗二：** 分析該框架在此期間針對**不同市場狀況**（牛/熊/盤）下的表現。  
...
* **重大疑點：** 分母真的是 **Sell Price**。  
  * 一般金融報酬率公式：$(P\_{sell} \- P\_{buy}) / P\_{buy}$。  
  * 論文公式：$(P\_{sell} \- P\_{buy}) / P\_{sell}$。  
* **這有什麼影響？**  
  * 如果股價翻倍 (100 \-\> 200)，一般公式報酬率是 100%；用這個公式算只有 50%。  
  * 如果股價腰斬 (100 \-\> 50)，一般公式報酬率是 \-50%；用這個公式算卻是 \-100% (50-100)/50。  
  * **結論：** 這個公式會**低估獲利，並放大虧損**。雖然這讓模型訓練變得更「嚴格」（保守），但在學術定義上是相當不尋常的。如果您要復現代碼，建議改回標準公式。
...
作者沒有迴避**手續費 (0.1%)** 和 **資金限制 (Max 10 stocks)**。很多學術論文會假設「無限資金」或「零手續費」，做出來的績效很漂亮，但一上線就賠錢。這篇論文在這方面做得比較紮實。
...
* **Pro Trader RL (本研究)：** 表現**顯著優於**所有其他策略和系統。  
...
* **其他模型表現：**  
  ...
  * **RL 基準模型 (PPO/DQN)：** 表現極差 (PPO 年化 \-12.72%)。這證明了直接把 PPO 丟進股市通常會失敗，必須像 Pro Trader RL 一樣進行**模組化設計**。
...
* **能源 (Energy)：** 平均回報最高 (**14.58%**)。顯示框架在**高波動**市場特別有效。  
...
* 雖然論文說涵蓋了 S\&P 400/600，但它是否考慮了**這 6 年間下市的股票**？如果只用「目前存在」的股票回測，績效會被高估。文中提到 "excluded stocks that did not have at least two years of data"，這是一個潛在的風險點。
...
這一部分非常關鍵。驗證一個交易策略好不好，不是看它在牛市賺多少（牛市大家都賺錢），而是看它在\*\*盤整（Sideways）**和**熊市（Bear）\*\*能不能活下來。
...
* **實驗結果 (參見 Table 9)：**  
  * **道瓊指數：** 表現溫和，年化報酬 **5.10%**。  
  * **唐奇安通道 (Donchian Channel)：** 表現**災難性**的差，年化報酬 **\-31.41%**，最大回撤 (MDD) 高達 **49.27%**。這反映了趨勢策略在盤整盤中的高風險。  
...
* **原因：** 唐奇安通道是「突破買進」。在盤整盤中，價格突破上緣通常是**假突破 (False Breakout)**，買進後馬上跌回區間，然後又跌破下緣觸發停損。這就是所謂的「被雙巴 (Whipsaw)」。  
...
**【圖表解析：Figure 7 (cite: image\_93384c.jpg)】**

* 圖 7 顯示了這半年的累積回報。  
* **關鍵轉折點：** 請看深藍色線 (Pro Trader RL)。在 3 月份崩盤時，它也跟著跌（這很正常，覆巢之下無完卵）。  
* **神奇的六月：** 但是從 **6 月中旬** 開始，藍線突然**旱地拔蔥**，垂直暴衝。這對應了美股當時的 V 型反轉行情。這暗示了 Pro Trader RL 在市場恐慌到底部時，成功抓到了反彈的主升段。
...
3. **互補效應 (Synergy)：** 論文最後一句話說得很好：「整合減輕了其他模組的風險和缺點。」這證明了這是一個成功的**集成學習 (Ensemble Learning)** 系統。
...
如果說熊市考驗的是**防守（Sell Agent & Stop Loss）**，那麼牛市考驗的就是**攻擊（Buy Agent）**。
...
* 圖 8 顯示了牛市期間的累積回報。  
* **橘線 (Buy Knowledge RL) 與 藍線 (Pro Trader RL)：** 這兩條線在初期就**垂直暴衝**。這顯示 AI 成功抓到了 2020 下半年到 2021 年初的那波科技股與權值股的主升段。  
...
* **Buy Knowledge RL：** 在**牛市**中提供主要的獲利來源（Upside）。  
* **Sell Knowledge RL：** 在**盤整與熊市**中扮演關鍵角色，提高風險調整後報酬並最小化損失。  
* **整合效應：** 結合「善於發現飆股的 Buy Agent」與「保守高效的 Sell Agent」，Pro Trader RL 實現了**正向的協同效應 (Positive Synergy)**。
...
論文的最後一個實驗 **Experiment 3** 通常是「神仙打架」——將 Pro Trader RL 與**最新的 SOTA (State-of-the-Art) 研究**進行對比。這通常是為了證明它不僅比舊方法好，還比同時代的其他論文好。

**您想看看它跟其他最新的 AI 交易模型比起來如何嗎？還是直接進入結論？**

\=======================================================================

這一段是 **實驗三 (Experiment 3\)**，也是這篇論文展示火力的最後一哩路。作者將 Pro Trader RL 拉出來跟其他學者發表的**最先進 (State-of-the-Art, SOTA)** 模型進行直接對比。
...
* **戰績：**  
  * **PMS\_CNN：** 累積報酬 **103.04%** (相當不錯)。  
  * **PMS\_RNN：** 累積報酬 37.74%。  
  * **Pro Trader RL (本研究)：** 累積報酬 **127.61%**，且夏普比率高達 **3.597** (PMS\_CNN 僅 0.578)。這證明了我們更穩定且報酬更高。
...
剩下的就是最後的 **5\. Conclusion (結論)** 與 **Future Work (未來展望)**。這部分通常會提到這個模型還有什麼**缺點**或者**可以改進**的地方。
...
透過實驗，我們發現 Pro Trader RL 框架的傑出表現歸功於其**整合式方法 (Integrated Approach)**，結合了買入知識 RL、賣出知識 RL 與停損規則模組。
...
1. **開發更先進的 RL Agent：** 將整合**多種投資策略**（目前只有唐奇安通道），以改善何時買賣的決策。  
...
這篇論文最大的貢獻不在於它用了什麼神級演算法（它只用了標準的 PPO），而在於它**拆解問題**的方式：
